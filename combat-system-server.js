...
calculateDamage(attacker, defender, attackArea, isBlocked) {
    ...
    // Check if this is a critical hit FIRST
    ...
    // NOW apply critical hit multiplier to the final damage after defense
    if (isCritical) {
        damage *= 2;
        console.log('Critical hit! Final damage doubled to:', damage);
    }
    ...
    checkEvasion(attacker, defender) {
          ...
          // Add defensive check for undefined defender
        if (!defender) {
        console.error('ERROR: Defender is undefined in checkEvasion');
        return false;
        }   
    ...
    playerIds.forEach(attackerId => {
        ...
        // Calculate damage - now returns an object with detailed info
        ...
        if (successfulBlock && damage === 0) {
            ...
            // Hit - use the actual critical flag, not a guess
            if (isCritical) {
                combatLog.push({
                    type: 'hit',
                    player: attackerId,
